...
class IFramePlayerOptions private constructor(private val playerOptions: JSONObject) {
    ...
    class Builder {
        companion object {
            ...
            private const val FRAMEBORDER = "frameborder"
            private const val ALLOWFULLSCREEN = "allowFullScreen"
        ...
        init {
            ...
            addString("playlist", "taJ60kskkns")
            addInt(REL, 0)
            addInt(FRAMEBORDER, 0)
            addString(ALLOWFULLSCREEN, "allowFullScreen")
            addInt(SHOW_INFO, 0)
            addInt(IV_LOAD_POLICY, 3)
            addInt(MODEST_BRANDING, 1)
            addInt(CC_LOAD_POLICY, 0)
            addInt(COLOR, Color.WHITE)
        }
